...
def csvdiplay(request):
    ...
    data = pd.read_csv(url) #讀取資料到data陣列中
    #資料欄互換
    data['SiteName'+'1']=data['SiteName']
    data['SiteName']=data['County']
    data['County']=data['SiteName1']
    data=data.drop(['SiteName1'], axis=1)
    datalen=len(data)
    datalilen=len(list(data))
    li=[]
    for i in range(0,len(data)):
        li.append('https://www.openstreetmap.org/search?query='+str(data['Longitude'].tolist()[i])+'%20'+str(data['Latitude'].tolist()[i])+'#map=10')
    return render(request,"csvdiplay.html",locals())